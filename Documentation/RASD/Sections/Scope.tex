...
Both municipalities and authorities must ask to SafeStreets operators to register their organization to the system before using the application, otherwise their members cannot be recognized and so cannot register or login to the system.

\subsubsection{Event analysis}
In the following we list the different kinds of events that can occur in the possible scenarios of interaction with SafeStreets. World events are phenomena occurring in the world that are not observable and not controllable by the SafeStreets' system. Shared events are phenomena that are either controlled by the world and observed by the SafeStreets' system or vice versa. Machine events are phenomena that are observed and controlled inside the SafeStreets' system, so they are not known to the world.
